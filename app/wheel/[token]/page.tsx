...
export default function WheelPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // Keep isFullscreen state in sync with actual fullscreen state
  useEffect(() => {
  ...
  const handleWheelReset = () => {
    ...
    // Fullscreen intentionally NOT exited here — stays stable for next user
  };
  ...
  if (!isRegistered) {
    return (
      <div className="spin-body min-h-screen flex items-center justify-center p-4">
        {/* Fullscreen Button on registration form */}
        <button
  ...
  return (
    <div className="spin-body min-h-screen flex flex-col justify-between items-center py-8 px-4 select-none">
      {/* Fullscreen Button — always visible on wheel screen */}
      <button
        onClick={toggleFullscreen}
        title={isFullscreen ? 'Exit Fullscreen' : 'Enter Fullscreen'}
        style={{
